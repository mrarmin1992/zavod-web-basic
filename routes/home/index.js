...
router.get('/sanitarni', (req, res)=>{
    res.render('home/sanitarni');
});
router.get('/ispravnost', (req, res)=>{
    res.render('home/ispravnost');
});
...
if(errors.length>0){
    ...
    User.findOne({email: req.body.email}).then(user=>{
        if(!user){
            ...
            req.flash('error_message', 'Email exist, please login');
         res.redirect('/login');
        }
    });
